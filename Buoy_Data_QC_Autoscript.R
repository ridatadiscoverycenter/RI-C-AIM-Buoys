...
#Pull the last week of data from the Marecotec API for Buoy-720
...
#Pull the last week of data from the Marecotec API for Buoy-620
...
global_range_data <- read.csv('C:/Users/kgomes1/Desktop/Buoy_QC_Reports/data_qc_global_range_values.csv', header=TRUE, sep=",", na.strings=c(""))
...
#Perform range check for applicable variables against historicalseasonal Narray Bay measurement range, flag with QC Flags (1=low, 2=high, 0=good) 
nb_hist_phys_data <- read.csv('C:/Users/kgomes1/Desktop/Buoy_QC_Reports/NB_PhysicalData_8.10.20201.csv', header=TRUE, sep=",", na.strings=c("","nd")) #Read in historical physical data from GSO LTS measurements
...
nb_hist_nutr_data <- read.csv('C:/Users/kgomes1/Desktop/Buoy_QC_Reports/nutrientdata_01.18.2021.csv', header=TRUE, sep=",", na.strings=c("","nd")) #Read in historical nutrient data from GSO LTS measurements
...
file_path_720 <- sprintf("C:/Users/kgomes1/Desktop/Buoy_QC_Reports/720_Weekly_Plots_%s.pdf",graph_date_range)
...
#Plot desired time series line plots in grid format.Lines are plotted for visual inspection, and colored by QC range flags.
file_path_620 <- sprintf("C:/Users/kgomes1/Desktop/Buoy_QC_Reports/620_Weekly_Plots_%s.pdf",graph_date_range)
...
#Setup gmailr Gmail api access
gm_auth_configure(path = "C:/Users/kgomes1/Desktop/Buoy_QC_Reports/client_secret_832741460937-vpo0nc1fiqrrl99mtf2b5dt0tficvp9i.apps.googleusercontent.com.json")
gm_auth(email = TRUE, cache ="C:/Users/kgomes1/Desktop/Buoy_QC_Reports/gmail_auth.secret")


qc_email_text <- sprintf("CAIM Buoy QC Report (%s)",graph_date_range)
qc_email <-
  gm_mime() %>%
  gm_to("") %>%
  gm_from("") %>%
  gm_subject(qc_email_text) %>%
  gm_text_body("Automated QC report for CAIM Buoys 720 and 620 ")%>% 
  gm_attach_file(file_path_720)%>% 
  gm_attach_file(file_path_620)

gm_send_message(qc_email)
